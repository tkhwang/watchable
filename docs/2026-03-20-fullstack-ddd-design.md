...
```typescript
// @tkbetter/shared/domain/value-objects/duration.ts
...
```typescript
// @tkbetter/shared/domain/value-objects/color.ts
...
```typescript
// @tkbetter/shared/domain/value-objects/time-range.ts
...
export class TimeRange {
  ...
  static fromJSON(raw: { startedAt: string; stoppedAt: string | null }): TimeRange {
    return new TimeRange(
      ...
      raw.stoppedAt ? new Date(raw.stoppedAt) : null,
    );
  }
...
```typescript
// @tkbetter/shared/domain/entities/task.ts
...
```typescript
// @tkbetter/shared/domain/entities/time-entry.ts
...
export class TimeEntry {
  ...
  get startedAt(): Date { return this._timeRange.startedAt; }
  get stoppedAt(): Date | null { return this._timeRange.stoppedAt; }
  get isRunning(): boolean { return this._timeRange.isRunning; }
  get updatedAt(): Date { return this._updatedAt; }
...
```typescript
// @tkbetter/shared/domain/entities/user-profile.ts
...
export class UserProfile {
  ...
  get pomodoroFocusMin(): number { return this._pomodoroFocusMin; }
  get pomodoroShortBreakMin(): number { return this._pomodoroShortBreakMin; }
  get pomodoroLongBreakMin(): number { return this._pomodoroLongBreakMin; }
  get updatedAt(): Date { return this._updatedAt; }
...
```typescript
// @tkbetter/shared/domain/services/timer-service.ts
...
## 6. Type Sharing: `@tkbetter/shared` Monorepo Package

### 결정

Domain 계층(Entity, VO, Domain Service)을 `@tkbetter/shared` 패키지에 위치시켜 모든 앱에서 공유한다.
