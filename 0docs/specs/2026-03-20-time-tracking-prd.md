...
| Shared   | `@watchable/shared`        | 도메인 모델, 타입, 유틸 |
...
- `@watchable/shared`에 Domain 계층을 위치시켜 모든 앱에서 공유한다.
...
현재 `@watchable` 모노레포가 구성되어 있으며, 아래 앱/패키지가 존재한다:

```
apps/mobile    # React Native Expo
apps/web       # Next.js
apps/api       # NestJS
packages/shared # @watchable/shared
```
